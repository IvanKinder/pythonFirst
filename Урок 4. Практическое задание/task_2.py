...
def new_reverse(number):  # быстрее, потому что ничего не приходится считать вообще, но без меморизации
    return str(number)[::-1]
...
'''
вариант с меморизацией ниже,
он быстр, но new_reverse быстрее,
думать пришлось меньше и код намного короче,
